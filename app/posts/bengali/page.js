...
const Page = () => {
  ...
  const fetchData = () => {
    ...
    if(status==='pending-approval'){
      url = `${process.env.NEXT_PUBLIC_API_URL}/posts/pending-approval/all?type=bengali&limit=${limit}&page=${currentPage}`;
    }else{
      url = `${process.env.NEXT_PUBLIC_API_URL}/posts/${status}?type=bengali&limit=${limit}&page=${currentPage}`;
    }
