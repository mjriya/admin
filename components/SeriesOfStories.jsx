    ...
    <div className="">
      ...
      {loading ? (
        ...
        <div className="flex flex-col justify-between gap-5">
          <div
            className={`${
              isDropdownOpen ? "h-[210px]" : "h-[310px]"
            }  text-zinc-700 flex flex-col w-full justify-start gap-1 `}
          >
            {series.map((article) => (
              <Link
                href={`/series/${article.title.split(" ").join("-")}--${
                  article._id
                }`}
                key={article._id}
                className={`text-start px-2 border-b border-yellow-800 rounded transition-all duration-200 hover:bg-zinc-100 ${
                  pathname.split("/")[1] === "series" &&
                  pathname.split("/")[2]?.split("--")[1] === article._id
                    ? "bg-yellow-50"
                    : "bg-white"
                }`}
              >
